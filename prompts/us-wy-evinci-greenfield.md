# Prompt — us-wy-evinci-greenfield

_Demonstrates: Microreactor — dry-cooled, tiny footprint; should unlock remote/off-grid land a large plant cannot use. Contrast with combo #1 same region._

**Paste everything below the line into ChatGPT. Return its JSON to Claude as `us-wy-evinci-greenfield`.**
...
Screen the candidate sites in **US-WY (USA)** for reactor **westinghouse-evinci**, pathway **greenfield**.
...
Return ONLY the AnalysisResult JSON for country="USA", regionId="US-WY", reactorId="westinghouse-evinci", pathway="greenfield".